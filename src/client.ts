...
import { Secret, SecretCreateParams, SecretDeleteResponse, SecretGetTotpResponse, SecretListResponse, Secrets } from './resources/secrets';
import { Task, TaskCreateParams, TaskDeleteResponse, TaskListResponse, TaskStartManualSessionParams, TaskStartManualSessionResponse, Tasks } from './resources/tasks';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['INDICES_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['INDICES_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Indices API. 
 */
export class Indices {
   ...
   * API Client for interfacing with the Indices API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['INDICES_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['INDICES_BASE_URL'] ?? https://api.indices.io] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('INDICES_BASE_URL'),
    apiKey = readEnv('INDICES_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.IndicesError(
        'The INDICES_API_KEY environment variable is missing or empty; either provide it, or instantiate the Indices client with an apiKey option, like new Indices({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.indices.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Indices.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('INDICES_LOG'), 'process.env[\'INDICES_LOG\']', this) ?? defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.indices.io';
  }
  ...
  static IndicesError = Errors.IndicesError;
