...
describe('resource tasks', () => {
  ...
  test.skip('startManualSession: request options and params are passed correctly', async () => {
    ...
    await expect(client.tasks.startManualSession('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { cookies: [{
    name: 'name',
    value: 'value',
    domain: 'domain',
    http_only: true,
    path: 'path',
    secure: true,
  }], use_proxy: true }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Indices.NotFoundError);
  });
});
