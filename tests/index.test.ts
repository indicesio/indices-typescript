...
import { APIPromise } from 'indicesio/core/api-promise';

import util from 'node:util';
import Indices from 'indicesio';
import { APIUserAbortError } from 'indicesio';
...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Indices({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'INDICES_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
