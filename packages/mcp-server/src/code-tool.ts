...
import { ClientOptions } from 'indicesio';

const prompt = `Runs JavaScript code to interact with the Indices API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    INDICES_API_KEY: requireValue(
      readEnv('INDICES_API_KEY') ?? client.apiKey,
      'set INDICES_API_KEY environment variable or provide apiKey client option',
    ),
    INDICES_BASE_URL: readEnv('INDICES_BASE_URL') ?? client.baseURL ?? undefined,
  };
