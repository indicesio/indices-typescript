...
const EMBEDDED_METHODS: MethodEntry[] = [
  {
    ...
    response:
      "{ id: string; created_at: string; creation: object; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: object; required_secrets?: object[]; }[]",
    markdown:
  ...
  },
  {
    ...
    endpoint: '/v1beta/tasks/{id}/start-manual-session',
    httpMethod: 'post',
    ...
    stainlessPath: '(resource) tasks > (method) start_manual_session',
    qualified: 'client.tasks.startManualSession',
    params: [
      'id: string;',
      'cookies?: { name: string; value: string; domain?: string; http_only?: boolean; path?: string; secure?: boolean; }[];',
      'use_proxy?: boolean;',
    ],
    response: '{ iframe_url: string; session_id: string; }',
    markdown:
      "## start_manual_session\n\n`client.tasks.startManualSession(id: string, cookies?: { name: string; value: string; domain?: string; http_only?: boolean; path?: string; secure?: boolean; }[], use_proxy?: boolean): { iframe_url: string; session_id: string; }`\n\n**post** `/v1beta/tasks/{id}/start-manual-session`\n\n<p>Spawn a browser session for manual task completion. If a session already exists, it will be closed and replaced.</p>\n\n### Parameters\n\n- `id: string`\n  The ID of the task to perform manually.\n\n- `cookies?: { name: string; value: string; domain?: string; http_only?: boolean; path?: string; secure?: boolean; }[]`\n  Initial cookies to set in the browser session.\n\n- `use_proxy?: boolean`\n  If true, spawn the browser session using a proxy.\n\n### Returns\n\n- `{ iframe_url: string; session_id: string; }`\n\n  - `iframe_url: string`\n  - `session_id: string`\n\n### Example\n\n```typescript\nimport Indices from 'indicesio';\n\nconst client = new Indices();\n\nconst response = await client.tasks.startManualSession('id');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.tasks.startManualSession',
        example:
          "import Indices from 'indicesio';\n\nconst client = new Indices({\n  apiKey: process.env['INDICES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.tasks.startManualSession('id');\n\nconsole.log(response.session_id);",
      },
      python: {
        method: 'tasks.start_manual_session',
        example:
          'import os\nfrom indices import Indices\n\nclient = Indices(\n    api_key=os.environ.get("INDICES_API_KEY"),  # This is the default and can be omitted\n)\nresponse = client.tasks.start_manual_session(\n    id="id",\n)\nprint(response.session_id)',
      },
      http: {
        example:
          'curl https://api.indices.io/v1beta/tasks/$ID/start-manual-session \\\n    -X POST \\\n    -H "Authorization: Bearer $INDICES_API_KEY"',
      },
    },
  },
  {
    name: 'complete_manual_session',
    endpoint: '/v1beta/tasks/{id}/complete-manual-session',
    httpMethod: 'post',
    summary: 'Complete manual session',
    description: '<p>Mark the manual browser session as complete and continue the task workflow.</p>',
    stainlessPath: '(resource) tasks > (method) complete_manual_session',
    qualified: 'client.tasks.completeManualSession',
    params: ['id: string;'],
    response:
      "{ id: string; created_at: string; creation: { secret_bindings?: object; secrets?: object[]; }; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: { category: string; message: string; }; required_secrets?: { name: string; type: 'login' | 'string'; requires_totp?: boolean; }[]; }",
    markdown:
      "## complete_manual_session\n\n`client.tasks.completeManualSession(id: string): { id: string; created_at: string; creation: task_creation; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: task_failure_info; required_secrets?: secret_slot_definition[]; }`\n\n**post** `/v1beta/tasks/{id}/complete-manual-session`\n\n<p>Mark the manual browser session as complete and continue the task workflow.</p>\n\n### Parameters\n\n- `id: string`\n  The ID of the task to perform manually.\n\n### Returns\n\n- `{ id: string; created_at: string; creation: { secret_bindings?: object; secrets?: object[]; }; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: { category: string; message: string; }; required_secrets?: { name: string; type: 'login' | 'string'; requires_totp?: boolean; }[]; }`\n\n  - `id: string`\n  - `created_at: string`\n  - `creation: { secret_bindings?: object; secrets?: { secret_id: string; description?: string; }[]; }`\n  - `current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'`\n  - `display_name: string`\n  - `input_schema: object`\n  - `output_schema: object`\n  - `task: string`\n  - `updated_at: string`\n  - `website: string`\n  - `failure_info?: { category: string; message: string; }`\n  - `required_secrets?: { name: string; type: 'login' | 'string'; requires_totp?: boolean; }[]`\n\n### Example\n\n```typescript\nimport Indices from 'indicesio';\n\nconst client = new Indices();\n\nconst task = await client.tasks.completeManualSession('id');\n\nconsole.log(task);\n```",
    perLanguage: {
      typescript: {
        method: 'client.tasks.completeManualSession',
        example:
          "import Indices from 'indicesio';\n\nconst client = new Indices({\n  apiKey: process.env['INDICES_API_KEY'], // This is the default and can be omitted\n});\n\nconst task = await client.tasks.completeManualSession('id');\n\nconsole.log(task.id);",
      },
      python: {
        method: 'tasks.complete_manual_session',
        example:
          'import os\nfrom indices import Indices\n\nclient = Indices(\n    api_key=os.environ.get("INDICES_API_KEY"),  # This is the default and can be omitted\n)\ntask = client.tasks.complete_manual_session(\n    "id",\n)\nprint(task.id)',
      },
      http: {
        example:
          'curl https://api.indices.io/v1beta/tasks/$ID/complete-manual-session \\\n    -X POST \\\n    -H "Authorization: Bearer $INDICES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; created_at: string; creation: { secret_bindings?: object; secrets?: object[]; }; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: { category: string; message: string; }; required_secrets?: { name: string; type: 'login' | 'string'; requires_totp?: boolean; }[]; }",
    markdown:
      "## retrieve\n\n`client.tasks.retrieve(id: string): { id: string; created_at: string; creation: task_creation; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: task_failure_info; required_secrets?: secret_slot_definition[]; }`\n\n**get** `/v1beta/tasks/{id}`\n\n<p>Retrieve a task by its ID.</p><p>For tasks that are still being generated, <code>input_schema</code> and <code>output_schema</code> may be <code>null</code>. They are guaranteed to be present once the task reaches the ready state.</p>\n\n### Parameters\n\n- `id: string`\n  The ID of the task to retrieve.\n\n### Returns\n\n- `{ id: string; created_at: string; creation: { secret_bindings?: object; secrets?: object[]; }; current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'; display_name: string; input_schema: object; output_schema: object; task: string; updated_at: string; website: string; failure_info?: { category: string; message: string; }; required_secrets?: { name: string; type: 'login' | 'string'; requires_totp?: boolean; }[]; }`\n\n  - `id: string`\n  - `created_at: string`\n  - `creation: { secret_bindings?: object; secrets?: { secret_id: string; description?: string; }[]; }`\n  - `current_state: 'not_ready' | 'waiting_for_manual_completion' | 'ready' | 'failed'`\n  - `display_name: string`\n  - `input_schema: object`\n  - `output_schema: object`\n  - `task: string`\n  - `updated_at: string`\n  - `website: string`\n  - `failure_info?: { category: string; message: string; }`\n  - `required_secrets?: { name: string; type: 'login' | 'string'; requires_totp?: boolean; }[]`\n\n### Example\n\n```typescript\nimport Indices from 'indicesio';\n\nconst client = new Indices();\n\nconst task = await client.tasks.retrieve('id');\n\nconsole.log(task);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: '<p>List runs for a given task.</p>',
    stainlessPath: '(resource) runs > (method) list',
    qualified: 'client.runs.list',
    params: ['task_id: string;', 'cursor?: string;', 'limit?: number;'],
    response:
      "{ id: string; arguments: object; created_at: string; finished_at: string; has_logs: boolean; result_json: string; status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'; task_id: string; secret_bindings?: object; }",
    markdown:
      "## list\n\n`client.runs.list(task_id: string, cursor?: string, limit?: number): { id: string; arguments: object; created_at: string; finished_at: string; has_logs: boolean; result_json: string; status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'; task_id: string; secret_bindings?: object; }`\n\n**get** `/v1beta/runs`\n\n<p>List runs for a given task.</p>\n\n### Parameters\n\n- `task_id: string`\n  The ID of the task to list runs for.\n\n- `cursor?: string`\n  Cursor from a previous response's `next_cursor`, to fetch the next page.\n\n- `limit?: number`\n  Maximum number of runs to return.\n\n### Returns\n\n- `{ id: string; arguments: object; created_at: string; finished_at: string; has_logs: boolean; result_json: string; status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'; task_id: string; secret_bindings?: object; }`\n\n  - `id: string`\n  - `arguments: object`\n  - `created_at: string`\n  - `finished_at: string`\n  - `has_logs: boolean`\n  - `result_json: string`\n  - `status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'`\n  - `task_id: string`\n  - `secret_bindings?: object`\n\n### Example\n\n```typescript\nimport Indices from 'indicesio';\n\nconst client = new Indices();\n\n// Automatically fetches more pages as needed.\nfor await (const run of client.runs.list({ task_id: 'task_id' })) {\n  console.log(run);\n}\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ id: string; arguments: object; created_at: string; finished_at: string; has_logs: boolean; result_json: string; status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'; task_id: string; secret_bindings?: object; }",
    markdown:
      "## retrieve\n\n`client.runs.retrieve(run_id: string): { id: string; arguments: object; created_at: string; finished_at: string; has_logs: boolean; result_json: string; status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'; task_id: string; secret_bindings?: object; }`\n\n**get** `/v1beta/runs/{run_id}`\n\n<p>Retrieve a run by its ID.</p>\n\n### Parameters\n\n- `run_id: string`\n  The ID of the run to retrieve.\n\n### Returns\n\n- `{ id: string; arguments: object; created_at: string; finished_at: string; has_logs: boolean; result_json: string; status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'; task_id: string; secret_bindings?: object; }`\n\n  - `id: string`\n  - `arguments: object`\n  - `created_at: string`\n  - `finished_at: string`\n  - `has_logs: boolean`\n  - `result_json: string`\n  - `status: 'pending' | 'running' | 'success' | 'failed' | 'timed_out' | 'result_too_large' | 'internal_error'`\n  - `task_id: string`\n  - `secret_bindings?: object`\n\n### Example\n\n```typescript\nimport Indices from 'indicesio';\n\nconst client = new Indices();\n\nconst run = await client.runs.retrieve('run_id');\n\nconsole.log(run);\n```",
    perLanguage: {
      typescript: {
